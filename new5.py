...
#==============================================================================#
                elif "ยกกลุ่ม" in msg.text.lower():
                    spl = re.split("ยกกลุ่ม",msg.text,flags=re.IGNORECASE)
                    if spl[0] == "":
                        spl[1] = spl[1].strip()
                        ag = nadya.getGroupIdsInvited()
                        txt = "👍กำลังยกเลิกการเชิญʕ•̫͡•ʔ "+str(len(ag))+" กลุ่ม"
                        if spl[1] != "":
                        ...
                        for gr in ag:
                          try:
                             ...
                             nadya.leaveGroup(gr)
                             nadya.sendMessage(msg.to,"👍ยกเลิกเสร็จสิ้นʕ•̫͡•ʔ")
                          except:
                             pass
                elif ".เนม " in msg.text.lower():
                    spl = re.split(".เนม ",msg.text,flags=re.IGNORECASE)
                    if [0] == "":
                       prof = nadya.getProfile()
                       prof.displayName = spl[1]
                       nadya.updateProfile(prof)
                       nadya.sendMessage(to, "👍เปลี่ยนชื่อเสร็จสิ้นʕ•̫͡•ʔ")
                    ...
                    nadya.sendMessage(msg.to,"👍เชิญเข้ากลุ่มการสนทนาแล้วʕ•̫͡•ʔ")
                elif 'speed' == msg.text.lower():
                    nadya.sendMessage(to, "👍การตอบสนองของบอทʕ•̫͡•ʔ")
                    start = time.time()
                    time.sleep(0.001)
                    ...
                    nadya.sendMessage(to,"👍บอทปกติดีไม่ได้หลุดʕ•̫͡•ʔ")
                    ...
                    if msg.toType == 2:
                        ...
                        nadya.sendMessage(to,"👍ยกเลิกค้างเชิญเสร็จสิ้นʕ•̫͡•ʔ️")
                elif "ลบรัน" == msg.text.lower():
                    gid = nadya.getGroupIdsInvited()
                    for i in gid:
                        nadya.rejectGroupInvitation(i)
                    if wait["lang"] == "JP":
                        nadya.sendText(msg.to,"👍ลบรันเสร็จสิ้นʕ•̫͡•ʔ")
                    else:
                    ...
                    time.sleep(5)
                    nadya.sendMessage(to, "รีบอทเสร็จสิ้น\n\nBotBy:✍️ͲɆᎪᎷ🔝ʕ•̫͡•ʔஞ௮Ҩஆี✨")
                    restartBot()
                elif text.lower() == 'ออน':
                    timeNow = time.time()
                    runtime = timeNow - botStart
                    runtime = format_timespan(runtime)
                    nadya.sendMessage(to, "ʕ•̫͡•ʔระยะเวลาการทำงานของบอทʕ•̫͡•ʔ\n➣͜{}".format(str(runtime)))
                    ...
                    try:
                        ...
                        ret_ += "\n╠══[  ฝึกแปลไทย ]"
                        ret_ += "\n╠ เวอร์ชั่น : 1.00"
                        ret_ += "\n╠ สร้างโดย: {}".format(creator.displayName)
                        ret_ += "\n╚══✍️ͲɆᎪᎷ🔝ʕ•̫͡•ʔஞ௮Ҩஆี✨\n"
